...
public class Board extends Observable implements TetrisControlls {    
    // myTimer and DEFAULT_DELAY_TIME are added by HUNG VU.
    // Class constants

    /**
     * Default interval of timer, the time requires for the piece to move to next position
     * (Interval: 1 second).
     */
    private static final int DEFAULT_DELAY_TIME = 1000; // added by Hung Vu
    ...
    /**
     * Timer for auto step (Interval: 1 second).
     */
    private Timer myTimer; // added by Hung Vu
    ...
    // The code below is add to make step become automatic.
    /**
     * This method is created to make Board's step become automatic
     * by using timer.
     * @param theEndStatus Timer behavior based on state of the game (GUI).
     */
    public void autostep(final boolean theEndStatus) {
        if (theEndStatus) {
            myTimer.cancel();
        } else {
            myTimer = new Timer();
